...
function initializeSlider() {
    slider.innerHTML = ''; // Удаляем все предыдущие слайды
    slides.forEach(slide => slider.appendChild(slide)); // Добавляем все оригинальные слайды
    showSlide(currentSlide);
}

function showSlide(index) {
    if (index >= slides.length) {
        index = slides.length - 1; // Останавливаемся на последнем слайде
    } else if (index < 0) {
        index = 0; // Останавливаемся на первом слайде
    }

    slider.style.transform = `translateX(-${index * 100}%)`;
    const slideNumber = document.getElementById('slide-number');
    currentSlide = index;
    slideNumber.textContent = `${index + 1} из ${slides.length}`;
}
...
// ... (Ваши другие функции и обработчики событий)
